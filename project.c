...
and the index in
// the table will always be the process ID. These choices waste
memory, but since this
// program is just a simulation it the easiest approach. Additionally,
debugging is
...
(runningState != -1). There is no need to schedule if a process is
...
// a. Mark the processing as running (update the new process's
PCB state)
...
runningState) to the blocked queue.
...
// 3. Update the running state to -1 (basically mark no process as
running). Note that a new process will be chosen to run later (via the
...
// 4. Update the running state to -1 (basically mark no process as
running). Note that a new process will be chosen to run later (via the
Q command code calling the schedule function).
}
...
// e. Set the priority to the same as the parent process's
priority.
...
that createProgram can fail if the file could not be opened or did not
exist.
...
// c. Change the state of the process to ready (update its PCB
entry).
